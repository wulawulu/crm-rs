...
impl CrmService {
    pub async fn try_new(config: AppConfig) -> Result<Self> {
        ...
        let notification = NotificationClient::connect(config.server.metadata_url.clone()).await?;
        let user_stats = UserStatsClient::connect(config.server.metadata_url.clone()).await?;
        Ok(Self {
            config,
            metadata,
            notification,
